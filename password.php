...
<html>
	<head>
       ...
       maxlength="20" required/>
							...
							if (($_POST['passwordreset']) == ($_POST['passwordreset2'])) {
							$passwordresetencrypted = password_hash(($_POST['passwordreset']), PASSWORD_DEFAULT);

							$reqpasswordreset = $bdd->prepare('UPDATE accounts SET password = :passwordchange WHERE id = :idlog');
							$reqpasswordreset->execute(array(
								'passwordchange' => $passwordresetencrypted,
								'idlog' => $_SESSION['id']
							));
							echo "<p style='color:green';><strong>Password updated !</strong></p>";

							} else {

								echo "<p style='color:red';><strong>Password dosen't match !</strong></p>";
							}
							?>
